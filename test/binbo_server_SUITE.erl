...
start_server_without_options(_Config) ->
    ...
    {ok, Opts} = binbo_bughouse:get_server_options(Pid),
...
test_bad_server_options(_Config) ->
    ...
    {error, {bad_server_options_type, _}} = binbo_bughouse:new_server(0),
    {error, {bad_server_options_type, _}} = binbo_bughouse:new_server(1.1),
    {error, {bad_server_options_type, _}} = binbo_bughouse:new_server(ok),
    {error, {bad_server_options_type, _}} = binbo_bughouse:new_server([]),
    {error, {bad_server_options_type, _}} = binbo_bughouse:new_server({a, 1}),
    {error, {bad_server_options_type, _}} = binbo_bughouse:new_server(<<"">>),
    {error, {bad_server_option, {unknown_option, value}}} = binbo_bughouse:new_server(#{unknown_option => value}),
    % set bad options
    {ok, Pid} = binbo_bughouse:new_server(),
    {error, {bad_server_options_type, _}} = binbo_bughouse:set_server_options(Pid, 0),
    {error, {bad_server_options_type, _}} = binbo_bughouse:set_server_options(Pid, 1.1),
    {error, {bad_server_options_type, _}} = binbo_bughouse:set_server_options(Pid, ok),
    {error, {bad_server_options_type, _}} = binbo_bughouse:set_server_options(Pid, []),
    {error, {bad_server_options_type, _}} = binbo_bughouse:set_server_options(Pid, {a, 1}),
    {error, {bad_server_options_type, _}} = binbo_bughouse:set_server_options(Pid, <<"">>),
    {error, {bad_server_option, {unknown_option, value}}} = binbo_bughouse:set_server_options(Pid, #{unknown_option => value}),

    {error,{not_pid,undefined}} = binbo_bughouse:stop_server(undefined),
...
start_server_with_idle_timeout(Config) ->
    IdleTimeout = 500,
    {ok, Pid} = binbo_bughouse:new_server(#{idle_timeout => IdleTimeout}),
...
get_set_server_options(Config) ->
    ...
    {ok, Pid} = binbo_bughouse:new_server(Opts1),

    % Get options
    {ok, Opts2} = binbo_bughouse:get_server_options(Pid),
    ...
    ok = binbo_bughouse:set_server_options(Pid, Opts3),
    {ok, Opts4} = binbo_bughouse:get_server_options(Pid),
    true = lists:all(fun(Key) ->
...
test_idle_timeout(Config) ->
    ...
    {ok, Pid} = binbo_bughouse:new_server(),
    ok = binbo_bughouse:set_server_options(Pid, #{
    ...
    ok = binbo_bughouse:set_server_options(Pid2, #{
        idle_timeout => NewIdleTimeout
    }),
    timer:sleep(NewIdleTimeout div 2),
    ...
    true = erlang:is_process_alive(Pid2), % still alive!
    ok = binbo_bughouse:stop_server(Pid2),
    ok.
...
onterminate_stop_server_undefined_game(_Config) ->
    Parent = self(),
    Ref = erlang:make_ref(),
    {ok, GamePid} = binbo_bughouse:new_server(#{
        onterminate => {fun onterminate_callback/4, {Parent, Ref}}
    }),
    ok = binbo_bughouse:stop_server(GamePid), % stop server
    ok = receive
...
onterminate_stop_server_initial_game(_Config) ->
    ...
    {ok, continue} = binbo_bughouse:new_game(GamePid),
    {ok, Fen0} = binbo_bughouse:get_fen(GamePid),
    ok = binbo_bughouse:stop_server(GamePid), % stop server
    ok = receive
        {GamePid, Reason, Game, {Parent, Ref}} ->
            normal = Reason,
...
onterminate_reset(_Config) ->
    ...
    {ok, continue} = binbo_bughouse:new_game(GamePid),
    ok = binbo_bughouse:set_server_options(GamePid, #{onterminate => undefined}),
    ok = binbo_bughouse:stop_server(GamePid), % stop server
    ok = receive
...
onterminate_bad_fun(_Config) ->
    {error, _} = binbo_bughouse:new_server(#{onterminate => not_fun}),
    {ok, GamePid} = binbo_bughouse:new_server(),
    {error, _} = binbo_bughouse:set_server_options(GamePid, #{
        onterminate => not_fun
    }),
    ok = binbo_bughouse:stop_server(GamePid),
    ok.
